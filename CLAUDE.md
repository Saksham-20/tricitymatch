...
**Mobile — missing backend (features hidden/disabled gracefully in RN until server side ships):** ⚪ **saved-searches** (no `/search/saved` route — Save-search UI hidden in `FilterPanel`/`SearchScreen`) · ⚪ **sent-interests** "My Interests" tab (no `/match/sent` — tab removed; backend exposes only daily/likes/shortlist/mutual) · ⚪ **education + income verification** (backend `Verification.documentType` enum is ID-only `aadhaar|pan|passport|driving_license` — RN tiers 3/4 show "Coming soon"; only Mobile+ID tiers are backed) · ⚪ **astrologer marketplace** uses stub data (`Astrologer` table empty/unseeded — real listing+booking backend exists but no data) · ⚪ RN i18n residual: 6 member screens still hardcoded EN (Notifications, Astrologer×2, HoroscopeMatch, Support, SuccessStory) + WelcomeScreen value-prop keys now added; admin/bureau/call screens EN (out of scope). Language switch itself works end-to-end (verified live hi/pa). RN icon/splash PNGs regenerated from website logo — apply on next `expo run:android` native build.
