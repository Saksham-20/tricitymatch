...
> **Active pass (2026-06-18):** Web **mobile-view** audit @375px — remove unprofessional slop (emoji in copy, childish colored info boxes, rose/pink/purple gradients, `✓`/`✦` unicode-as-icon), fix mobile layout + workflow bugs, screen-by-screen (fix-on-discovery, never bulk). Tracker: `mobile-qa-progress.md`. **RN apps deferred to a later pass** (web mobile view only for now).
