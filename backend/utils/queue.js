...
// Lifecycle mail jobs live in their own module so they can be tested without
...
const scheduleCleanupJobs = async () => {
  if (cleanupQueue) {
    ...
    // Lifecycle mail — every 30 minutes, offset from the :00/:30 jobs above.
    // The cadence is NOT how often anyone is mailed: each job only sends inside
    ...
    // Bull keys a repeatable job by name + cron, so changing a cron string
    // does not replace the old schedule — it adds a second one that keeps
    // firing from Redis until removed. Retire the earlier cadences explicitly.
    const retired = {
      'subscription-lifecycle': ['30 * * * *'],
      'photo-nudge': ['0 11 * * *'],
    };
    for (const job of await cleanupQueue.getRepeatableJobs()) {
      const stale = retired[job.name];
      if (stale && stale.includes(job.cron)) {
        await cleanupQueue.removeRepeatableByKey(job.key);
        log.info('Removed retired repeatable job', { name: job.name, cron: job.cron });
      }
    }
