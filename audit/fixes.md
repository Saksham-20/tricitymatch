...
| 4 | C4-1 | Keyboard-accessible profile cards (div onClick → Link / role=button+key handler) | pages/Dashboard.jsx, components/cards/MatchCard.jsx | _pending_ | ✅ live: MatchCard name = focusable `<a href>`, receives focus, navigates; build+31/31 green |
| 4 | C4-2 | Shortlist heart `aria-label` + `aria-pressed` + `type=button` + focus ring | pages/Dashboard.jsx | _pending_ | ✅ build+31/31 green |
