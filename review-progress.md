...
**Result:** build ✅, FE 31/31, BE 99 pass (9 pre-existing/stale). **Backend audit 0 high/0 critical** (27 moderate transitive uuid-chain). **Frontend audit: 0 vulnerabilities** (the clean lockfile rebuild + RTL 16 / vitest 4 pulled patched glob/minimatch). **React pinned to 18.2.0 via root override** so the workspace keeps a single React copy (mobile/RN locks 18.2.0; a floating range nested a 2nd copy → dual-React).
...
## Build-Work Completion Pass (2026-06-16) — deferred items now DONE
- **Stale unit tests — DONE.** errorHandler tests realigned to shipped API (`error.code`, nested `{error:{}}`, config-mock for dev/prod stack), AppError default statusCode 500; searchValidation now enforces cross-field ageMin≤ageMax + heightMin≤heightMax (real validators added). Backend unit suite **116/116 green** (was 99 + 9 failing).
- **REF-41 / SEC-4 full — DONE.** `validateUploadedFiles` (`middlewares/upload.js`) now magic-byte-validates disk-stored uploads (reads leading 12 bytes vs `MAGIC_BYTES`, deletes mismatches), wired on profile-photo + verification-doc routes; Cloudinary-streamed files stay decode-validated by `allowed_formats`. Unit test `uploadMagicBytes.test.js` (5 tests). (Avoided the full memory-storage refactor — covers the only unvalidated path, the disk fallback.)
- **REF-51 / MF-1 — DONE (built, not hidden).** Family-group chat backend: `models/Group|GroupMember|GroupMessage`, migration **000039** (applied live), `controllers/groupController.js` + `routes/groupRoutes.js` mounted at `/groups`, all handlers gated by `requireMembership` (closes IDOR). Socket `join-group` re-enabled with membership check; group writes are REST-only + server-authoritative broadcasts (`group-message-received/-edited/-deleted`). Mobile `api/chat.ts` + Family-group screens rewired from the never-existed `/chat/family-groups` to `/groups` (mobile tsc clean for these files). Unit test `groupAuth.test.js` (membership gate). SOCK-1/SOCK-2 fully resolved (no longer just mitigated).
- **Kundli PDF — DONE.** `utils/kundli.js` (pdfkit) renders Ashtakoot guna table + Manglik + numerology + summary; `GET /profile/:id/horoscope-match/pdf` (premium); web download button on ProfileDetail.
- **Web in-browser calls — DONE (config-gated).** `agora-rtc-sdk-ng` (lazy-imported → own 1.5MB chunk, not in initial bundle); `context/CallContext.jsx` + `components/calls/CallOverlay.jsx` + `utils/agoraEngine.js` + `api/calls.js`; voice/video buttons on ProfileDetail; socket signaling (call-incoming/accepted/declined/ended already existed). Functions when `VITE_AGORA_APP_ID` + Agora server creds set; UI auto-hides otherwise.
- **Web i18n — PARTIAL→broader.** Global Navbar + Login wired to react-i18next; `navbar` + `auth` key namespaces added en/hi/pa. Remaining content pages (Home/About/Contact/Safety/Privacy/Terms) still English — deliberate (large content-translation effort, low eng risk).
- **Still deferred:** held dep migrations (react 19 mobile-blocked, tailwind 4, express 5, multer 2, umzug 3, eslint 10); remaining-page i18n; backend integration/e2e (need live DB).
