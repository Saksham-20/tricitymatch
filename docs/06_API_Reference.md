...
Returns `ashtakoot: null` if either user's nakshatra is missing — `rashiScore` is populated from rashi fallback.
...
| `group-message-received` | `{ groupId, message }` | New family group message. |
