    ...
    <motion.div className="space-y-5" initial="initial" animate="animate" variants={staggerContainer}>
      ...
      <motion.div variants={fadeRise}>
        ...
        <textarea
          ...
          maxLength={500}
          rows={4}
          className="w-full px-4 py-3 border border-neutral-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-primary-500 focus:border-transparent transition-colors duration-[160ms] resize-none"
        />
            ...
            <AnimatePresence initial={false}>
              {formData.interestTags.map((interest) => (
                <motion.div
                  ...
                  exit="exit"
                  variants={listRow}
                  className="inline-flex items-center gap-2 bg-primary-100 text-primary-800 px-3 py-1 min-h-[36px] rounded-full text-sm font-medium"
                >
                  {interest}
                  <button
                    type="button"
                    ...
                    className="hover:text-primary-900 transition-colors duration-[160ms]"
                  >
                    <FiX size={16} />
                  </button>
                </motion.div>
        ...
        <div className="flex flex-wrap gap-2">
          {INTERESTS.map((interest) => (
            <button
              ...
              className={`px-3 py-2 min-h-[44px] rounded-lg font-medium text-sm transition-colors duration-[160ms] active:scale-[0.97] ${
          ...
          {LANGUAGES.map((lang) => {
            ...
            return (
              <button
                ...
                className={`px-3 py-2 min-h-[44px] rounded-lg font-medium text-sm border transition-colors duration-[160ms] active:scale-[0.97] ${
                  selected
